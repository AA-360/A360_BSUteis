...
public final class LetterCalcCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(LetterCalcCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    LetterCalc command = new LetterCalc();
    ...
    if(convertedParameters.containsKey("value")) {
      if(convertedParameters.get("value") != null) {
        try {
          Pattern pattern = Pattern.compile("\\w{1}");
          Matcher matcher = pattern.matcher((String) convertedParameters.get("value"));
          if(!matcher.matches()) {
            throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.RegExMatchNoExample","value",""));
          }
        }
        catch(ClassCastException e) {
          throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","value", "String", convertedParameters.get("value").getClass().getSimpleName()));
        }
    ...
    try {
      Optional<Value> result =  Optional.ofNullable(command.action((String)convertedParameters.get("value"),(Double)convertedParameters.get("number")));
