...
public class TableQuery {
    ...
    public ListValue<String> action(
            ...
            @Pkg(label = "é uma query Mysql", description = "Formata o texo no formato apropriado")
            @NotEmpty
            Boolean isMysql,
        ...
        for(String sc:fnd.shemaNames()){
            if(this.countOccurrences(fnd.shemaNames(),sc)>1){
    ...
    private int countOccurrences(List<String> arr, String x)
    {
        int res = 0;
        for (String oc: arr)
            if (x == oc)
