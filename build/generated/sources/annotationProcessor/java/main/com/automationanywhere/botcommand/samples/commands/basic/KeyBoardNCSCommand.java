...
public final class KeyBoardNCSCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(KeyBoardNCSCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    KeyBoardNCS command = new KeyBoardNCS();
    ...
    if(parameters.containsKey("status") && parameters.get("status") != null && parameters.get("status").get() != null) {
      ...
      if(convertedParameters.get("status") !=null && !(convertedParameters.get("status") instanceof Boolean)) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","status", "Boolean", parameters.get("status").get().getClass().getSimpleName()));
      }
    }
    ...
    try {
      command.action((String)convertedParameters.get("select"),(Boolean)convertedParameters.get("status"));Optional<Value> result = Optional.empty();
